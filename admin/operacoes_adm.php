<link rel="stylesheet" href="../css/estilo.css">
<main class="admin-container-adm">
  <h1></h1>
  <h1 class="admin-title-adm">Área Administrativa</h1>

  <div class="caixa-tela-adm">

    <!-- ADM PRODUTOS -->
    <div class="caixa-adm">
      <div class="card-adm">
        <img src="../img/produto.svg" alt="Imagem de Produtos" class="card-img-adm">
        <div class="card-body-adm">
          <h5 class="card-tit-adm">PRODUTOS</h5>
          <h1></h1>
          <div class="card-botão-adm">
            <a href="produtos/lista_produtos.php" class="btn-adm btn-produtos">LISTAR</a>
            <a href="produtos/insere_produtos.php" class="btn-adm btn-produtos">INSERIR</a>
          </div>
        </div>
      </div>
    </div>

    <!-- ADM CATEGORIA -->
    <div class="caixa-adm">
      <div class="card-adm">
        <img src="../img/categoria.svg" alt="Imagem de Categoria" class="card-img-adm">
        <div class="card-body-adm">
          <h5 class="card-tit-adm">CATEGORIA</h5>
          <h1></h1>
          <div class="card-botão-adm">
            <a href="categorias/lista_categorias.php" class="btn-adm btn-categorias">LISTAR</a>
            <a href="categorias/insere_categorias.php" class="btn-adm btn-categorias">INSERIR</a>
          </div>
        </div>
      </div>
    </div>

    <!-- ADM USUARIOS CLIENTES-->
    <div class="caixa-adm">
      <div class="card-adm">
        <img src="../img/user.svg" alt="Imagem de Produtos" class="card-img-adm">
        <div class="card-body-adm">
          <h5 class="card-tit-adm">CLIENTES</h5>
          <h1></h1>
          <div class="card-botão-adm">
            <a href="usuarios-cli/lista_usuarios-cli.php" class="btn-adm btn-usuarios-cli">LISTAR</a>
            <a href="usuarios-cli/insere_usuarios-cli.php" class="btn-adm btn-usuarios-cli">INSERIR</a>
          </div>
        </div>
      </div>
    </div>
    
    <!-- ADM USUARIOS FUNCIONARIOS-->
    <div class="caixa-adm">
      <div class="card-adm">
        <img src="../img/user.svg" alt="Imagem de Produtos" class="card-img-adm">
        <div class="card-body-adm">
          <h5 class="card-tit-adm">FUNCIONÁRIOS</h5>
          <h1></h1>
          <div class="card-botão-adm">
            <a href="usuarios-fun/lista_usuarios-fun.php" class="btn-adm btn-usuarios-fun">LISTAR</a>
            <a href="usuarios-fun/insere_usuarios-fun.php" class="btn-adm btn-usuarios-fun">INSERIR</a>
          </div>
        </div>
      </div>
    </div>

  </div>
</main>
