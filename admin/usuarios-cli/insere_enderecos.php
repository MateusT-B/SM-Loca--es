...
if ($_POST) {
    ...
    if ($resultadoEndereco) {
        ...
        $stmtClienteEndereco = $conn->prepare("INSERT INTO clientes_enderecos (id_cliente, id_endereco) VALUES (?, ?)");
        $stmtClienteEndereco->bind_param('ii', $idCliente, $idEndereco);
        $resultadoClienteEndereco = $stmtClienteEndereco->execute();

        if ($resultadoClienteEndereco) {
            // 4. Redireciona para a próxima página ou mostra uma mensagem de sucesso
            header('Location: insere_contatos.php?id_cliente=' . $idCliente);
            exit;
        } else {
            echo "Erro ao associar endereço ao cliente: " . $conn->error;
            exit;
        }
    } else {
